...
Shader "Shader Forge/BaseToonLit" {
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform float _DistanceMul_copy;
            uniform float _DistanceAmplify;
            struct VertexInput {
...
////// Lighting:
////// Emissive:
                float3 emissive = (lerp(lerp(_bottom_color.rgb,_top_color.rgb,clamp(((i.uv0.g*_height_blur)+_base_height),0.0,1.0)),(_rim_color.rgb*_EmitPower),clamp((((_top_rim_blur*i.uv0.g)+_top_rim_offset)*pow(1.0-max(0,dot(normalDirection, viewDirection)),_rim_power)),0.0,1.0))*i.vertexColor.rgb*((1.0 - exp((saturate((partZ*_DistanceMul_copy*0.001))*(-1.0))))*_DistanceAmplify)*_OverallBrightness);
