...
////// Emissive:
                ...
                float2 node_595 = (i.uv0+node_1976.g*float2(0.01,0.01));
                float4 _WaveNoise_var = tex2D(_WaveNoise,TRANSFORM_TEX(node_595, _WaveNoise));
                float2 node_3111 = (i.uv0+node_1976.g*float2(0.05,0.02));
                ...
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(((node_3378*node_9884)+float3(i.uv0,0.0)), _MainTex));
                ...
                float2 node_6168 = ((i.uv0+node_1976.g*float2(0,-0.1))*0.5);
                float4 _ShimmerNoise_var = tex2D(_ShimmerNoise,TRANSFORM_TEX(node_6168, _ShimmerNoise));
                float3 emissive = (((_MainTex_var.rgb*_Color.rgb*i.vertexColor.rgb)*node_603)+(_ShimmerColor.rgb*_ShimmerTex_var.a*(_ShimmerNoise_var.r*_ShimmerNoisePower)));
